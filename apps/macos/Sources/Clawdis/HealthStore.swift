...
final class HealthStore {
    ...
    var state: HealthState {
        ...
        if !snap.web.linked {
            ...
            return Self.isTelegramHealthy(snap) ? .degraded("WhatsApp not linked") : .linkingNeeded
        }
    ...
    var summaryLine: String {
        ...
        if !snap.web.linked {
            if let tg = snap.telegram, tg.configured {
                ...
                return "\(tgLabel) · WhatsApp not linked — run clawdis login"
            }
            return "WhatsApp not linked — run clawdis login"
        }
